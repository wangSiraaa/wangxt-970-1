...
const LAWYERS = [
  { id: 'L001', name: '张伟', specialties: ['civil', 'contract'], licenseStatus: 'active', phone: '13800001001' },
  { id: 'L002', name: '李芳', specialties: ['family', 'civil'], licenseStatus: 'active', phone: '13800001002' },
  { id: 'L003', name: '王刚', specialties: ['criminal', 'admin'], licenseStatus: 'active', phone: '13800001003' },
  { id: 'L004', name: '赵敏', specialties: ['labor', 'civil'], licenseStatus: 'active', phone: '13800001004' },
  { id: 'L005', name: '陈磊', specialties: ['property', 'contract'], licenseStatus: 'active', phone: '13800001005' },
  { id: 'L006', name: '刘洋', specialties: ['injury', 'criminal'], licenseStatus: 'suspended', phone: '13800001006' },
  { id: 'L007', name: '孙婷', specialties: ['admin', 'labor'], licenseStatus: 'active', phone: '13800001007' },
  { id: 'L008', name: '周峰', specialties: ['family', 'property'], licenseStatus: 'active', phone: '13800001008' },
];
...
export function checkInterestConflict(citizenIdCard, lawyerId, caseType) {
  const conflicts = [];
...
export function createAppointment(booking, operator = '群众') {
  ...
  const conflicts = checkInterestConflict(booking.citizenIdCard, booking.lawyerId, booking.caseType);
  const apt = {
  ...
  addAuditLog('创建预约', operator, `${booking.citizenName} 预约律师 ${booking.lawyerId} ${booking.date} ${booking.timeSlot}，案件类型: ${booking.caseType}`);
  notify();
  return { success: true, appointment: apt, errors, conflicts };
}

export function cancelAppointment(id, operator = '群众') {
  const apt = data.appointments.find((a) => a.id === id);
  if (!apt) return false;
  apt.status = 'cancelled';
  addAuditLog('取消预约', operator, `取消预约 ${id}，群众: ${apt.citizenName}`);
...
export { TIME_SLOTS, CASE_TYPES, MATERIALS, LAWYERS, RECUSAL_RELATIONS };
